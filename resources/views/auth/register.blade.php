<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Name -->
        <div>
            <x-input-label for="name" :value="__('Nama Pemilik / Penanggung Jawab')" />
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        
        <div class="mt-4">
            <x-input-label for="phone" :value="__('No. WhatsApp Aktif')" />
            <x-text-input id="phone" class="block mt-1 w-full" type="number" name="phone" :value="old('phone')" required />
            <x-input-error :messages="$errors->get('phone')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="domicile" :value="__('Domisili (Kota/Kab)')" />
            <x-text-input id="domicile" class="block mt-1 w-full" type="text" name="domicile" :value="old('domicile')" required />
