<x-app-layout>
  <x-slot name="header">
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-gray-600 mt-1">Perbarui judul, caption, atau ganti file promosi di form ini</p>
      </div>

      <a href="{{ route('marketing-kits.index') }}" class="flex items-center gap-2 text-gray-600 hover:text-gray-900 font-medium">
  ...
  <style>
    :root{
      --page-max-w:880px; --card-radius:16px; --border:#e6e7eb; --muted:#6b7280; --accent:#4f46e5; --success:#10b981;
    }
    body { background: linear-gradient(180deg,#f7fbff 0,#ffffff 100%); }

    .form-container { max-width: var(--page-max-w); margin: 2.25rem auto; padding: 1.25rem; }
    ...
    .form-header { padding:1.25rem 1.5rem; display:flex; gap:.75rem; align-items:center; background: linear-gradient(135deg,#667eea 0,#764ba2 100%); color:white; border-bottom:1px solid #f3f4f6; }
    ...
    .file-upload { position:relative; margin-top:.5rem; }
    .file-input { position:absolute; width:0.1px; height:0.1px; opacity:0; z-index:-1; }
    .file-label { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:12px; padding:28px 20px; border-radius:14px; border:2.5px dashed #d1d5db; background:#fbfdff; cursor:pointer; text-align:center; transition:background .18s, border-color .18s, transform .12s; }
    ...
    .image-preview, .media-preview { margin-top:1rem; text-align:center; }
    .preview-container { max-width:420px; margin:0 auto; position:relative; }
    .preview-image, .preview-video { width:100%; height:280px; object-fit:cover; border-radius:12px; border:1px solid var(--border); box-shadow:0 8px 20px rgba(2,6,23,0.04); background:#000; }
  ...
  <script>
    (function () {
      ...
      function showPreviewImage(src) {
        ...
        btn.onclick = () => { clearPreview(); fileInput.value = ''; };
    ...
    (function () {
      ...
      form.addEventListener('submit', function (e) {
        ...
        if (!title.value || !String(title.value).trim()) { alert('Judul wajib diisi.'); title.focus(); e.preventDefault(); return; }
        if (!caption.value || !String(caption.value).trim()) { alert('Caption wajib diisi.'); caption.focus(); e.preventDefault(); return; }
